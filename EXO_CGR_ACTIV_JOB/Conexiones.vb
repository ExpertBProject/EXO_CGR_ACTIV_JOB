                    ...
                    Case XmlNodeType.Element
                        Select Case Reader.Name.ToString.Trim
                            Case "DI"
                                ...
                                oCompany.CompanyDB = Reader.GetAttribute("database").ToString.Trim
...
#Region "Connect to SQL Server"
    Public Shared Sub Connect_SQLServer(ByRef db As SqlConnection, ByVal sTipoSQL As String)
        Dim myStream As Stream = Nothing
        Dim Reader As XmlTextReader = Nothing

        Try
            'Conectar SQL
            myStream = File.OpenRead("./Connections.xml")
            'myStream = File.OpenRead(My.Application.Info.DirectoryPath.ToString & "\Connections.xml")
            Reader = New XmlTextReader(myStream)
            myStream = Nothing
                        ...
                        Select Case Reader.Name.ToString.Trim
                            Case sTipoSQL
                                If db Is Nothing OrElse db.State = ConnectionState.Closed Then
                                    ...
                                    db.ConnectionString = "Database=" & Reader.GetAttribute("Db").ToString.Trim & ";Data Source=" & Reader.GetAttribute("Server").ToString.Trim & ";User Id=" & Reader.GetAttribute("DbUser").ToString & ";Password=" & Reader.GetAttribute("DbPwd").ToString
    ...
    Public Shared Function GetValueDB(ByRef db As SqlConnection, ByRef sTabla As String, ByRef sCampo As String, ByRef sCondicion As String) As System.Data.DataTable
